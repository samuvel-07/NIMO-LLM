...
# Import visual integration for Three.js WebSocket
from visual_integration import get_visual_broadcaster, set_visual_state
...
class JarvisCore(threading.Thread):
    def __init__(self, log_fn=print):
        ...
        # Initialize visual WebSocket broadcaster
        try:
            self.visual = get_visual_broadcaster()
            self.log("Visual WebSocket broadcaster initialized")
        except Exception as e:
            self.log(f"Visual broadcaster warning: {e}")
            self.visual = None
    ...
    def speak(self, text):
        # Visual: RESPONDING state when speaking
        set_visual_state("RESPONDING", audio_level=0.5)
        _speak_text(self.engine, text, self.log)
        # Back to current state after speaking
